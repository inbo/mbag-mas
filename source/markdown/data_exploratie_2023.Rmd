...
relfreqs_vogels <- mas_2023_vogels %>%
  mutate(stratum = paste(openheid_klasse, sbp, sep = " - ")) %>%
  st_drop_geometry() %>%
  # Only keep breeding birds exept for predators
...
relfreqs_vogels %>%
  ...
  arrange(desc(naam), desc(gemiddelde_freq)) %>%
...
relfreqs_doelsoorten <- relfreqs_vogels %>%
  ...
  mutate(naam = reorder(naam, order_var,
                        decreasing = FALSE)) %>%
  select(-c(n, w)) %>%
  rename(gemiddelde_freq_regio = order_var)
...
relfreqs_vogels %>%
  ...
  full_join(relfreqs_doelsoorten %>%
              distinct(landbouwstreek, naam, gemiddelde_freq_regio),
            by = join_by(landbouwstreek, naam)) %>%
  mutate(naam = reorder(naam, gemiddelde_freq_regio,
                        decreasing = FALSE)) %>%
  ggplot() +
    geom_vline(xintercept = 0, alpha = 0.2, linetype = "dashed") +
...
$I(\cdot)$ is de indicatorfunctie die 1 geeft als $n_i > 0$ en 0 anderzijds.
We selecteerden enkel waarnemingen met broedcode groter dan nul.
Voor roofzoogdieren hielden we geen rekening met broedcode, omdat we voor deze soorten meer algemeen geïnteresseerd zijn in predatiedruk op de akkerzoogdieren.
