...
relfreq_vogels_ratio <- relfreqs_vogels %>%
  ...
  mutate(ratiofreq = `Zandleemstreek` / `Leemstreek`) %>%
  ungroup() %>%
  arrange(desc(ratiofreq)) %>%
  rownames_to_column(var = "rang") %>%
  mutate(rang = as.numeric(rang)) %>%
  select(rang, naam, Zandleemstreek, Leemstreek, ratiofreq)
...
relfreq_ratio_zn <- relfreqs_vogels %>%
  ...
  full_join(relfreq_vogels_ratio %>%
              select(rang, naam, ratiofreq_mean = ratiofreq),
            by = join_by(naam)) %>%
  filter(!is.infinite(ratiofreq_mean)) %>%
...
relfreq_ratio_zn %>%
  ...
  geom_point(aes(x = ratiofreq, y = naam),
    size = 1
  ) +
  labs(
...
relfreqs_vogels %>%
  ...
  mutate(ratiofreq = `Leemstreek` / `Zandleemstreek`) %>%
  ungroup() %>%
  filter(!is.infinite(ratiofreq)) %>%
  slice_max(ratiofreq, n = 20) %>%
  kable(digits = 5)
...
# Calculate relative frequency per region per period as weighted mean
# Calculate ratio between regions
relfreq_ratio_total <- relfreqs_vogels %>%
  group_by(landbouwstreek, jaar, periode_in_jaar, naam) %>%
  summarise(gemiddelde_freq = weighted.mean(relfreq, w)) %>%
  ungroup() %>%
  pivot_wider(
...
p <- relfreq_ratio_total %>%
  ...
  geom_hline(
    yintercept = c(
      10.5,
      length(unique(relfreq_ratio_total$naam)) - 10 + 0.5
    ),
    alpha = 0.2, linetype = "dashed"
  ) +
  geom_point(aes(x = ratiofreq, y = naam),
  ...
  labs(y = "", x = "Ratio van relatieve frequenties") +
  scale_y_discrete(labels = function(x) highlight(x, doelsoorten, "red")) +
  coord_cartesian(xlim = c(
    0 - abs(max(relfreq_ratio_zn$ratiofreq)),
    0 + abs(max(relfreq_ratio_zn$ratiofreq))
  )) +
  scale_x_continuous(breaks = seq(-1, 1, 0.05)) +
  theme(
  ...
  # Add arrows to indicate direction commonness in regions
  annotate("segment",
    x = c(abs(max(relfreq_ratio_zn$ratiofreq))),
    y = c(length(unique(relfreq_ratio_total$naam)) - 10 + 1),
    xend = c(abs(max(relfreq_ratio_zn$ratiofreq))),
    yend = c(length(unique(relfreq_ratio_total$naam)) - 10 + 3),
    arrow = arrow(type = "closed", length = unit(0.015, "npc")),
    colour = "black"
  ) +
  annotate("segment",
  ...
  annotate("text",
    label = c("Zandleemstreek"),
    x = c(abs(max(relfreq_ratio_zn$ratiofreq)) + 0.01),
    y = c(length(unique(relfreq_ratio_total$naam)) - 10 + 1),
    size = 2, hjust = "left", colour = "black", angle = 90
  ) +
  annotate("text",
...
save_figure(p, here(media_folder, "vogelfreqs_ratio_2023"), devices,
  dpi = 300, width = 8, height = 6
)
