...
gewichten_za <- steekproefkader_kp %>%
  count(regio, openheid_klasse, sbp) %>%
  group_by(openheid_klasse) %>%
...
gewichten_za <- steekproefkader_po %>%
  count(regio, openheid_klasse, sbp) %>%
  group_by(openheid_klasse) %>%
...
Km_45192.7 was oorspronkelijk VL0432?
Km_14981.5 was oorspronkelijk VL0005?

```{r}
steekproef_avimap_za_kp_po <- avimap_za_kp_po_final %>%
  select(pointid, regio = Naam) %>%
  mutate(pointid = case_match(pointid,
    "Km_45192.7" ~ "VL0432",
    "Km_14981.5"~ "VL0005",
    .default = pointid
  )) %>%
