---
title: "MAS steekproefontwerp zandstreek"
...
De code maakt een intersectie tussen landbouwstreek Zandstreek (heel België) en contour Vlaanderen en voegt deze toe  aan file `za_kp_po_perimeters.gpkg`.
...
#   st_transform(crs = 31370)
#
# zand_vlaanderen <- st_intersection(landbouwstreken, vlaanderen) |>
#   filter(NAAM == "Zandstreek") |>
#   mutate(section = NA) |>
#   select(Naam = NAAM, section, geom = geometry)
#
# st_write(obj = zand_vlaanderen,
...
telcirkels_selectie_lgb <- tar_read(telcirkels_selectie_landgebruik,
  ...
  filter(startsWith(pointid, abbreviate("Zandstreek", 2)))
```
...
Het oppervlakte aandeel aan SBP's binnen de Zandstreek:
...
Het oppervlakte aandeel aan SBP's binnen OL en HOL in de Zandstreek:
...
# Trekking steekproef Zandstreek
...
-   Hier moeten we afweging maken tussen de precisie per stratum en precisie voor de Zandstreek in zijn geheel.
...
Er zijn 4 strata (OL en HOL: binnen en buiten SBP) binnen de Zandstreek.
...
De eerste bv. 300 (laagste volgnummers) zullen dan de set zijn voor de Zandstreek, die kan verdeeld worden over de medewerkers.
...
mapview(
  ...
  mapview(
    cirkels_zandstreek %>% filter(pointid %in% intersect$pointid),
    layer = "nieuwe punten"
  )
```
...
steekproef_out %>%
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_zandstreek_telpunten.geojson"
    ),
    delete_dsn = TRUE
  )
...
steekproef_out %>%
  ...
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_zandstreek_telpunten.kml"
    ),
    delete_dsn = TRUE
  )
...
steekproef_out %>%
  ...
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_zandstreek_telcirkels.geojson"
    ),
    delete_dsn = TRUE
  )
...
steekproef_out %>%
  ...
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_zandstreek_telcirkels.kml"
    ),
    delete_dsn = TRUE
  )
```
...
```{r}
zandstreek_avimap_df <- read_csv(file.path(
  "data",
  "processed",
...
```{r, eval=FALSE}
zandstreek_avimap_sf %>%
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_zandstreek_avimap.gpkg"
    ),
    delete_dsn = TRUE
  )
...
```{r, eval=FALSE}
git2rdata::write_vc(zandstreek_avimap_df,
  file = file.path(
    "data",
    "processed",
    "steekproef_zandstreek_avimap"
  ),
  sorting = "pointid",
  strict = FALSE
)
