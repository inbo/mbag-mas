...
targets_store <- here("source", "targets", "mas_steekproef_weidestreek", "_targets")
```
...
mapview(perimeters_data,
  ...
  mapview(paden, legend = FALSE) +
  mapview(exclusie_punten, col.regions = "black",
          legend = FALSE)
```
...
balansvergelijking <- steekproefkader_finaal %>%
  ...
  mutate(puntenset = factor(puntenset, levels = c("volledig steekproefkader", "steekproef")))
...
weidestreek_avimap_df <- read_csv(file.path(
  "data",
  "processed",
  "MAS netwerk Vlaanderen- steekproef_weidestreek_telpunten-final.csv"
))
