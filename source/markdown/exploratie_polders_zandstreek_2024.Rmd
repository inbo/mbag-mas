---
title: "Exploratie Polders en Zandstreek 2024"
...
media_folder <- here("media", "exploratie_pl_zn_2024")
...
save_figure(p, here(media_folder, "tellingen_vogels_pl_zn_2024"), devices,
...
De telpunten zijn in alle periodes geteld.
...
save_figure(p, here(media_folder, "soorten_vogels_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 6
)
...
save_figure(p, here(media_folder, "waarnemingen_vogels_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 6
)
...
gewichten_pl <- read_csv(
  here("data", "steekproefkaders", "gewichten_polders.csv")
)
gewichten_zn <- read_csv(
  here("data", "steekproefkaders", "gewichten_zandstreek.csv")
)
```
...
gewichten_mas_2024 <- bind_rows(
  ...
  select(landbouwstreek, regio, openheid_klasse, sbp, n, w = prop)
...
save_figure(p, here(media_folder, "cum_soorten_vogels_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 8
)
```

Een overzicht van het aantal soorten per telpunt in de Polders en de Zandstreek.
...
save_figure(
  p,
  here(media_folder, "aantal_soorten_vogels_strat_pl_zn_2024"),
...
save_figure(p, here(media_folder, "vogelfreqs_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 6
)
...
relfreqs_vogels %>%
  ...
  write_csv(here(media_folder, "relfreq_vogels_diff_pl_zn_2024.csv"))
```
...
Top 20 van soorten die in termen van verschil in gemiddelde relatieve frequentie van voorkomen meer voorkomen in de Polders dan in de Zandstreek:
...
relfreqs_vogels %>%
  ...
  mutate(verschilfreq = `Polders` - `Zandstreek`) %>%
  ungroup() %>%
  slice_max(verschilfreq, n = 20) %>%
  kable(digits = 2)
```
...
save_figure(p, here(media_folder, "vogelfreqs_diff_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 6
)
...
Top 20 van soorten die in termen van ratio in gemiddelde relatieve frequentie van voorkomen meer voorkomen in de Zandstreek dan in de Polders (uitgesloten soorten die slechts in 1 van de 2 landbouwstreken voorkomen):
...
relfreq_vogels_ratio <- relfreqs_vogels %>%
  ...
  mutate(ratiofreq = `Zandstreek` / `Polders`) %>%
  ungroup() %>%
  arrange(desc(ratiofreq), desc(Zandstreek), Polders) %>%
  rownames_to_column(var = "rang") %>%
  mutate(rang = as.numeric(rang),
         ratio_zn_lm = ratiofreq,
         ratio_lm_zn = 1 / ratiofreq) %>%
  select(rang, naam, Zandstreek, Polders, ratio_zn_lm, ratio_lm_zn)

write_csv(relfreq_vogels_ratio, here(media_folder,
                                     "relfreq_vogels_ratio_pl_zn_2024.csv"))
```
...
relfreq_ratio_zn <- relfreqs_vogels %>%
  ...
  mutate(ratio_zn_lm = `Zandstreek` / `Polders`) %>%
  # Retrieve 20 most common species + goal species
...
save_figure(p, here(media_folder, "vogelfreqs_ratio_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 6
)
...
Top 20 van soorten die in termen van ratio in gemiddelde relatieve frequentie van voorkomen meer voorkomen in de Polders dan in de Zandstreek (uitgesloten soorten die slechts in 1 van de 2 landbouwstreken voorkomen):
...
relfreq_ratio_lm <- relfreqs_vogels %>%
  ...
  mutate(ratio_lm_zn = `Polders` / `Zandstreek`) %>%
  # Retrieve 20 most common species + goal species
  full_join(relfreq_vogels_ratio %>%
...
De volgende soorten zijn enkel in de Polders gezien.
...
save_figure(p, here(media_folder, "vogelfreqs_pl_zn_2024_doelsoorten"), devices,
...
write_csv(summary_zoogdieren_2024,
          here(media_folder, "summary_zoogdieren_pl_zn_2024.csv"))
...
save_figure(
  p,
  here(media_folder, "waarnemingen_zoogdieren_pl_zn_2024"),
  devices,
  dpi = 300, width = 8, height = 6
)
...
Voor verdere analyses groeperen we de Westelijke en Oostelijke polders tot de Polders.
...
save_figure(p, here(media_folder, "cum_soorten_zoogdieren_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 8
)
```
...
write_csv(
  ...
  here(media_folder, "relfreq_zoogdieren_pl_zn_2024.csv")
)
...
save_figure(p, here(media_folder, "zoogdierfreqs_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 6
)
...
write_csv(
  ...
  here(media_folder, "relfreq_zoogdieren_diff_pl_zn_2024.csv")
)
...
save_figure(p, here(media_folder, "zoogdierfreqs_diff_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 6
)
...
save_figure(p, here(media_folder, "aantal_hazen_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 6
)
...
save_figure(p, here(media_folder, "relfreq_haas_pl_zn_2024"), devices,
  dpi = 300, width = 8, height = 6
)
