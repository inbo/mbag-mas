...
Als we voor de eerste beste match gaan, zien we verschillende fouten optreden:
...
- Roodborst is een insect die ook die Nederlandse naam heeft
- Eekhoorn is een ondersoort
- Merel is een slang die merel in zijn wetenschappelijke soortnaam heeft
- Hop is een virus
- Haas is een genus die haas in auteur heeft
- Veldmuis/Aardmuis matched met Aardmuis

We nemen de default limit (100).
We zijn niet tevreden met eerste beste match, maar gaan met een specifieke regex door meerdere vernacular names zoeken (Merel zal bv. ook niet met Indische merel matchen).
...
We specificeren dat we enkel soortnamen willen:
...
Voor Haas vinden we geen goeie match. Deze naam komt te vaak terug in soortnamen/auteurs. We verhogen de limit:
...
species_matching_mas_list <- lapply(
  species_names_mas_list, map_taxa_from_vernacular,
  vernacular_name_col = "dwc_vernacularName",
  out_cols = c("scientificName", "family", "class"),
  filter_cols = list(class = "dwc_class", kingdom = "dwc_kingdom"),
  lang = "nld",
...
test_wet_names <- rbind.data.frame(species_matching_mas2 %>%
                   filter(!is.na(scientificName)),
                 species_matching_mas_rest2) %>%
...
missing_soorten_wet_mas %>%
  ...
  mutate(naam = ifelse(euring == 10170, "Gele kwikstaart (spec)", naam)) %>%
...
2. Match met soorten van andere continenten omdat onze soortnaam ook aan die buitenlandse soort gekoppeld is
  - Roodborsttapuit van Afrikaanse roodborsttapuit
  - Staartmees van Birmese staartmees
  - Watersnip van Amerikaanse watersnip
  - Wintertaling van Amerikaanse wintertaling
  - Zilvermeeuw van Amerikaanse zilvermeeuw
3. Random
- Brandgans van Bergeend


```{r}
map_taxa_from_vernacular(
  species_names_mas %>% filter(dwc_vernacularName == "Roodborsttapuit"),
  vernacular_name_col = "dwc_vernacularName",
  out_cols = c("scientificName", "family", "class"),
  filter_cols = list(class = "dwc_class", kingdom = "dwc_kingdom"),
  lang = "nld",
...
- We missen nog een paar belangrijke kolommen
...
```
col_order <- c("type", "language", "license", "rightsHolder", "accessRights",
               ...
               "occurrenceRemarks" ,"samplingProtocol", "samplingEffort",
               ...
               "scientificName", "kingdom", "scientificNameAuthorship",
