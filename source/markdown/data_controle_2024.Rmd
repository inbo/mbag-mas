...
## Amersfoort naar Lambert coördinaten (crs_pipeline)

In de eerste stap worden Amersfoort coördinaten naar Lambert coördinaten omgezet.
...
removed_obs <- select_species_groups %>%
  ...
  anti_join(st_drop_geometry(remove_double_counts),
            by = join_by(oid, projectid, plotid, plotnaam, x_amersfoort,
                         y_amersfoort, x_lambert, y_lambert, soortgrp, soortnr,
...
Zie ook `dwc_mapping_mas.Rmd` voor de initiële exploratie.
...
dwc_mapping_final %>%
  rowwise() %>%
  mutate(ndecim_lat = decimalplaces(decimalLatitude),
         ndecim_long = decimalplaces(decimalLongitude),
         ndecim_max = sum(ndecim_lat, ndecim_long)) %>%
  ungroup() %>%
  slice_max(ndecim_max, with_ties = FALSE) %>%
  select(ndecim_lat, ndecim_long, decimalLatitude, decimalLongitude) %>%
  kable()
```

> ok, maximum 6 cijfers na de komma

```{r}
dwc_mapping_final %>%
  rowwise() %>%
  mutate(ndecim_lat = decimalplaces(verbatimLatitude),
         ndecim_long = decimalplaces(verbatimLongitude),
         ndecim_max = sum(ndecim_lat, ndecim_long)) %>%
  ungroup() %>%
  slice_max(ndecim_max, with_ties = FALSE) %>%
  select(ndecim_lat, ndecim_long, verbatimLatitude, verbatimLongitude) %>%
  kable()
```

> ok, maximum 1 cijfer na de komma

`taxonID`:

```{r}
dwc_mapping_final %>%
  filter(vernacularName %in% c("Haas", "Veldleeuwerik")) %>%
  distinct(vernacularName, taxonID, scientificName) %>%
  kable()
```
...
```{r}
observer_lookup <- read_csv(
  file.path(mbag_dir, "source", "targets", "data_preparation",
            "data", "observer_lookup.csv"),
  show_col_types = FALSE
)
dwc_mapping_final %>%
  count(recordedBy, identifiedBy) %>%
  full_join(observer_lookup, by = join_by(recordedBy == observer_id)) %>%
  head() %>%
  kable()
```

Hebben we evenveel unieke waarden in de dataset als in de tabel?

```{r}
dwc_mapping_final %>%
  count(recordedBy, identifiedBy) %>%
  nrow() == nrow(observer_lookup)
```
...
```{r}
dwc_mapping_final %>%
  distinct(behavior, verbatimBehavior, organismQuantityType, lifeStage) %>%
  kable()
```
...
test_wet_names <- manual_taxon_data %>%
  ...
  mutate(
    ...
    euring = as.numeric(sub("^0+", "", sub("^euring:", "", dwc_taxonID)))
  ) %>%
  rowwise() %>%
  mutate(
  ...
  left_join(soorten_wet_mas, by = join_by(euring)) %>%
  select(-"groepnr")
```
...
- Opsplitsen in open en vervaagde datasets
  ...
  - Vervagen (UTM 5 km)
    ...
    - kwetsbare soorten alle waarnemingen --> opheffing nooit
  - Embargo
    - data kwetsbare soorten niet vrijgeven voor 2 jaar (telkens vanaf 1 januari)
  - Nieuwe kolommen in vervaagde dataset
    - `informationWithheld`: "original locations available upon request" <!-- spell-check: ignore -->
...
# 2 Check: all vulnerable species are blurred
check_vuln_full <- dwc_mapping_final %>%
  filter(tolower(vernacularName) %in% tolower(vulnerable_species),
         eventDate < embargo_date) %>%
  left_join(dwc_mapping_final_blurred, by = "occurrenceID",
            suffix = c("_orig", "_blurred")) %>%
  mutate(
...
> De eerste waarneming ligt toevallig op dezelfde `decimalLatitude` en de tweede op dezelfde `verbatimLatitude`. Waarom bij eerste `verbatimLatitude` niet en in tweede `decimalLatitude` niet? Dit is omwille van de projectie.
