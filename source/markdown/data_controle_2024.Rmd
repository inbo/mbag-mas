...
data <- outside_observations_new %>%
  filter(regio != "Oostelijke leemstreek", regio != "Polders")
...
for (j in sort(unique(data$jaar))) {
  data <- outside_observations_new %>%
    filter(regio != "Oostelijke leemstreek", regio != "Polders", jaar == j)
...
for (j in sort(unique(data$jaar))) {
  data <- outside_observations_new %>%
    filter(regio == "Oostelijke leemstreek", jaar == j)
