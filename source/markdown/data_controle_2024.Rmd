...
We doen dit per jaar.

```{r}
write_breeding_codes <- mas_data_full %>%
  st_drop_geometry() %>%
  filter(aantal > 1,
         wrntype > 0) %>%
  select(-"broedcode") %>%
...
for (j in sort(unique(write_breeding_codes$jaar))) {
  write_breeding_codes %>%
    filter(jaar == j) %>%
    write_csv(file.path(mbag_dir, "output", "data_controle",
                        paste0("hoge_aantallen_broedcodes_", j, ".csv")))
}
```
