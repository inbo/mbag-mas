...
```{r}
for (reg in sort(unique(outside_observations_new$regio))) {
  p <- outside_observations_new %>%
      filter(regio == reg) %>%
      distinct(regio, plotnaam, jaar, periode_in_jaar, outlier_perc) %>%
      mutate(outlier_perc = round(outlier_perc, 1)) %>%
      ggplot() +
        geom_bar(aes(x = outlier_perc), width = 0.1) +
...
```{r, results='asis'}
data <- outside_observations_new %>%
          filter(regio != "Oostelijke leemstreek",
                 regio != "Polders")

for (j in sort(unique(data$jaar))) {
  data <- outside_observations_new %>%
          filter(regio != "Oostelijke leemstreek",
                 regio != "Polders",
                 jaar == j)
      ...
      mapview(
        outside_plots_new %>%
          filter(regio != "Oostelijke leemstreek",
                 ...
                 jaar == j,
                 periode_in_jaar == ronde),
        cex = 3, col.regions = "black", legend = FALSE
        ) +
      mapview(
        outside_observations_new %>%
          filter(regio != "Oostelijke leemstreek",
                 ...
                 jaar == j,
                 periode_in_jaar == ronde),
        zcol = "outlier", layer = "Locatie t.o.v. telcirkel")
...
We zien een problematisch punt: Pl_49572. Alle andere lijken ok.
