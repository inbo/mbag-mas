...
Zijn het alle Vl punten?

```{r}
vl_points_outlying <- outlying_distances %>%
  filter(grepl("^VL", plotnaam)) %>%
  distinct(plotnaam) %>%
  pull(plotnaam)

all_vl_points <- calculate_obs_distance %>%
  st_drop_geometry() %>%
  filter(grepl("^VL", plotnaam)) %>%
  distinct(plotnaam) %>%
  pull(plotnaam)

identical(sort(vl_points_outlying), sort(all_vl_points))
```

Zijn het alle VL punten in de Oostelijke leemstreek?
...
We kijken naar telronde 1 in 2024.

```{r}
outlying_plots <- outlying_distances %>%
  filter(grepl("^VL", plotnaam)) %>%
  distinct(plotnaam) %>%
  pull(plotnaam)

# Test two locations
observations_test1 <- select_time_periods %>%
  filter(plotnaam %in% outlying_plots,
         jaar == 2024,
         periode_in_jaar == "R1")

observations_test2 <- calculate_obs_distance %>%
  filter(plotnaam %in% outlying_plots,
         jaar == 2024,
         periode_in_jaar == "R1")

# Calculate buffered observations
buffered_observations1 <- observations_test1 %>%
  st_buffer(observations_test1$distance2plot)

buffered_observations2 <- observations_test2 %>%
  st_buffer(observations_test2$distance2plot)

# Get problematic point locations
plots_test <- observations_test2 %>%
  st_drop_geometry() %>%
  distinct(regio, plotnaam, jaar, periode_in_jaar, x_coord, y_coord) %>%
  st_as_sf(coords = c("x_coord", "y_coord"), crs = 31370)

# Get problematic location circles
circles_test <- plots_test %>%
  st_buffer(300)
```

Met de herberekende afstanden zien we dat de buffers rond de waarnemingen mooi in het telpunt samenkomen.

```{r}
# Visualise normal case
mapview(observations_test2, col.regions = "red", legend = FALSE) +
  mapview(buffered_observations2,
          alpha.regions = 0, color = "black", legend = FALSE) +
  mapview(plots_test,
          col.regions = "black", legend = FALSE) +
  mapview(circles_test,
          legend = FALSE)
```

In welke punten komen de originele afstanden samen?

```{r}
# Visualise normal case
mapview(observations_test2, col.regions = "red", legend = FALSE) +
  mapview(buffered_observations1,
          alpha.regions = 0, color = "black", legend = FALSE) +
  mapview(plots_test,
          col.regions = "black", legend = FALSE) +
  mapview(circles_test,
...
Het telpunt in Avimap lijkt ergens anders te liggen.
De punten in Avimap liggen inderdaad op de kruisingen van de buffers.
Er is dus iets mis met de coördinaten in de data.

Het gaat blijkbaar om telpunten die vervangen worden door reeds bestaande punten
De locatie is het nieuwe telpunt, maar de naam is die van het reeds bestaande punt, waardoor er een mismatch heb tussen de locaties van de telpunten in de steekproef en de locaties van de data via Sovon.
Waarschijnlijk bij eerste tellingen van pilootproject punten meegegeven waarbij de telpuntnaam was aangepast naar het reeds bestaande punt, maar niet de locatie.
Het vreemde is dat deze mismatch al voorkomt vanaf 2018 (van vóór het pilootproject).

> Namen van telpunten aanpassen in Avimap van al deze punten?
...
- VL0289, 2020, R3, JJNN16: ok, laatste telling
- VL0330, 2021, R2, NOVN00: ok, laatste telling
...
We schrijven de data voor Kalkoen weg.
...
- Dataset preparatie
  - We verwijderen van kolommen gerelateerd aan steekproef
...
test_wet_names <- manual_taxon_data %>%
  ...
  rowwise() %>%
  mutate(scientificName = gsub(authorship, "", scientificName),
...
- Hoeveel zetten we voor `coordinateUncertaintyInMeters`? Voorlopig op 5 m gezet.
...
- Wat is het verschil tussen `individualCount` en `organismQuantity`?
- We kunnen ook afwezigheden toevoegen (`occurrenceStatus = "absent"` en `individualCount = 0`) aangezien alle vogels worden genoteerd op het moment van tellen vanaf dat telpunt. Hoe doen we dit?
