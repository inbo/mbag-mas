---
title: "MAS steekproefontwerp polders"
...
De code maakt een intersectie tussen landbouwstreek polders (heel België) en contour Vlaanderen en voegt deze toe  aan file `za_kp_po_perimeters.gpkg`.
...
#   st_transform(crs = 31370)
#
# pold_vlaanderen <- st_intersection(landbouwstreken, vlaanderen) |>
#   filter(NAAM == "Polders") |>
#   mutate(section = NA) |>
#   select(Naam = NAAM, section, geom = geometry)
#
# st_write(obj = pold_vlaanderen,
...
telcirkels_selectie_lgb <- tar_read(telcirkels_selectie_landgebruik,
  ...
  filter(startsWith(pointid, abbreviate("Polders", 2)))
```
...
Het oppervlakte aandeel aan SBP's binnen de Polders:
...
Het oppervlakte aandeel aan SBP's binnen OL en HOL in de Polders:
...
-   Hier moeten we afweging maken tussen de precisie per stratum en precisie voor de Polders in zijn geheel.
...
Er zijn 4 strata (OL en HOL: binnen en buiten SBP) binnen de Polders.
...
De eerste bv. 300 (laagste volgnummers) zullen dan de set zijn voor de Polders, die kan verdeeld worden over de medewerkers.
...
We vervangen alle telpunten uit de steekproef van de Polders door reeds bestaande punten indien ze voor minstens 50 % overlappen en ze tot hetzelfde stratum behoren.
...
cirkels_polders <- steekproef_raw %>%
  st_buffer(300)
```
...
intersect <- st_intersection(oude_cirkels_buffer, cirkels_polders) %>%
  mutate(
...
steekproef_out %>%
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_polders_telpunten.geojson"
    ),
    delete_dsn = TRUE
  )
...
steekproef_out %>%
  ...
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_polders_telpunten.kml"
    ),
    delete_dsn = TRUE
  )
...
steekproef_out %>%
  ...
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_polders_telcirkels.geojson"
    ),
    delete_dsn = TRUE
  )
...
steekproef_out %>%
  ...
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_polders_telcirkels.kml"
    ),
    delete_dsn = TRUE
  )
```
...
```{r, eval=FALSE}
polders_avimap_df <- read_csv(file.path(
  "data",
  "processed",
...
polders_avimap_sf %>%
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_polders_avimap.gpkg"
    ),
    delete_dsn = TRUE
  )
...
polders_avimap_sf %>%
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_polders_avimap.geojson"
    ),
    delete_dsn = TRUE
  )
```
...
```{r, eval=FALSE}
git2rdata::write_vc(polders_avimap_df,
  file = file.path(
    "data",
    "processed",
    "steekproef_polders_avimap"
  ),
  sorting = "pointid"
)
```
