...
targets_store <- here("source", "targets", "mas_steekproef_zn_lm", "_targets")

# to make tar_read work
...
De code maakt een intersectie tussen landbouwstreek leemstreek (heel België) en contour Vlaanderen en voegt deze toe  aan file "zandleemstreek_perimeters.gpkg".
...
```{r, class.source = "fold-show", eval=FALSE}
# Code uitvoeren in mas_steekproef_zn_lm.Rproj
# na laden van globals in targets_steekproef_zn_lm.Rmd
...
temp <- file %>%
  ...
  select(OIDN, UIDN, ALVID = REF_ID, HFDTLT = GWSCOD_H, LBLHFDTLT = GWSNAM_H,
         GEWASGROEP = GWSGRPH_LB, PM = GESP_PM, LBLPM = GESP_PM_LB,
         LENGTE, OPPERVL)

sfarrow::st_write_parquet(obj = temp,
                          dsn = file.path(mbag_dir,
                                          "data",
                                          "landbouwgebruikspercelen",
                                          "parquet",
                                          "lbgbrprc2022.parquet"),
                          chunk_size = 10000)
```
...
lbg_statistieken %>%
  ...
  pivot_longer(
    cols = c(prop_n_points, mean_prop, mean_prop_present),
    names_to = "variable",
    values_to = "value"
  ) %>%
  ggplot() +
  geom_point(aes(
    y = forcats::fct_reorder(name, value, mean),
...
-   graangewassen (monocotylen zoals tarwe, gerst, e.a.)
-   grasland (opdelen permanent en tijdelijk)
-   mais
-   aardappelen
-   wortelgewassen (biet, cichorei, wortel)
-   peulvruchten (erwt)
-   groenteteelt (kolen zoals bloemkool, broccoli, spruiten, prei, ajuin, sla)
-   groenbemesters (klavers, luzerne, mosterd) of koolzaad
-   meerjarige fruitteelt (appel, peer, kers, e.a.)
-   vlas
-   andere (bv eenjarig fruitteelt)
