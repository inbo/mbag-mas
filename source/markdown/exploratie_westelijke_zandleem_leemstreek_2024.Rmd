...
relfreq_vogels_regio <- relfreqs_vogels %>%
  ...
  select(naam, `Westelijke leemstreek`, Zandstreek,
...
Top 20 van soorten die in termen van verschil in gemiddelde relatieve frequentie van voorkomen meer voorkomen in de Zandstreek dan in de Westelijke leemstreek:
...
relfreqs_vogels %>%
  ...
  mutate(verschilfreq = `Zandstreek` - `Westelijke leemstreek`) %>%
  ungroup() %>%
...
relfreqs_vogels %>%
  ...
  rownames_to_column(var = "rang") %>%
  mutate(rang = as.numeric(rang)) %>%
  select(rang, naam, Zandstreek, `Westelijke leemstreek`, verschilfreq) %>%
...
relfreq_diff_zn %>%
  ...
  labs(
    ...
    title = "Algemener in de Zandstreek"
  ) +
  scale_y_discrete(labels = function(x) highlight(x, doelsoorten, "red")) +
...
Top 20 van soorten die in termen van verschil in gemiddelde relatieve frequentie van voorkomen meer voorkomen in de Westelijke leemstreek dan in de Zandstreek:
...
relfreqs_vogels %>%
  ...
  mutate(verschilfreq = `Westelijke leemstreek` - `Zandstreek`) %>%
  ungroup() %>%
  slice_max(verschilfreq, n = 20) %>%
  kable(digits = 2)
```
...
Top 20 van soorten die in termen van ratio in gemiddelde relatieve frequentie van voorkomen meer voorkomen in de Zandstreek dan in de Westelijke leemstreek (uitgesloten soorten die slechts in 1 van de 2 landbouwstreken voorkomen):
...
relfreq_vogels_ratio <- relfreqs_vogels %>%
  ...
  mutate(ratiofreq = `Zandstreek` / `Westelijke leemstreek`) %>%
  ungroup() %>%
  arrange(desc(ratiofreq), desc(Zandstreek), `Westelijke leemstreek`) %>%
  rownames_to_column(var = "rang") %>%
  mutate(rang = as.numeric(rang),
         ratio_zn_lm = ratiofreq,
         ratio_lm_zn = 1 / ratiofreq) %>%
  select(rang, naam, Zandstreek, `Westelijke leemstreek`, ratio_zn_lm, ratio_lm_zn)
...
relfreq_ratio_zn <- relfreqs_vogels %>%
  ...
  mutate(ratio_zn_lm = `Zandstreek` / `Westelijke leemstreek`) %>%
  # Retrieve 20 most common species + goal species
...
De volgende soorten zijn enkel in de Zandstreek gezien.
...
Top 20 van soorten die in termen van ratio in gemiddelde relatieve frequentie van voorkomen meer voorkomen in de Westelijke leemstreek dan in de Zandstreek (uitgesloten soorten die slechts in 1 van de 2 landbouwstreken voorkomen):
...
relfreq_ratio_lm <- relfreqs_vogels %>%
  ...
  mutate(ratio_lm_zn = `Westelijke leemstreek` / `Zandstreek`) %>%
  # Retrieve 20 most common species + goal species
  full_join(relfreq_vogels_ratio %>%
...
relfreqs_doelsoorten <- relfreqs_vogels %>%
  ...
  select(naam, `Westelijke leemstreek`, Zandstreek, gewogen_gemiddelde = order_var)
...
p <- relfreqs_vogels %>%
  ...
  full_join(
    relfreqs_doelsoorten %>%
      pivot_longer(
        cols = c("Westelijke leemstreek", "Zandstreek"),
