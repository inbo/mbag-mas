---
title: "Exploratie Westelijke Leemstreek en Westelijke Zandleemstreek 2024"
...
We hebben een beperkt aantal telpunten binnen sbp in de Westelijke leemstreek.
...
relfreqs_vogels %>%
  ...
  mutate(
    verschilfreq = `Westelijke zandleemstreek` - `Westelijke leemstreek`
  ) %>%
  ungroup() %>%
...
relfreqs_vogels %>%
  ...
  rownames_to_column(var = "rang") %>%
  mutate(rang = as.numeric(rang)) %>%
  select(rang, naam, `Westelijke zandleemstreek`, `Westelijke leemstreek`,
         verschilfreq) %>%
...
relfreqs_vogels %>%
  ...
  mutate(
    verschilfreq = `Westelijke leemstreek` - `Westelijke zandleemstreek`
  ) %>%
  ungroup() %>%
  slice_max(verschilfreq, n = 20) %>%
  kable(digits = 2)
...
relfreq_diff_total <- relfreqs_vogels %>%
  ...
  filter(
    as.numeric(naam) > length(unique(naam)) - 10 |
      as.numeric(naam) <= 10 |
      naam %in% doelsoorten
  )
...
relfreq_vogels_ratio <- relfreqs_vogels %>%
  ...
  ungroup() %>%
  arrange(desc(ratiofreq), desc(`Westelijke zandleemstreek`),
...
relfreq_ratio_zn <- relfreqs_vogels %>%
  ...
  mutate(
    ratio_zn_lm = `Westelijke zandleemstreek` / `Westelijke leemstreek`
  ) %>%
  # Retrieve 20 most common species + goal species
...
relfreq_ratio_lm <- relfreqs_vogels %>%
  ...
  mutate(
    ratio_lm_zn = `Westelijke leemstreek` / `Westelijke zandleemstreek`
  ) %>%
  # Retrieve 20 most common species + goal species
  full_join(relfreq_vogels_ratio %>%
...
save_figure(p, here(media_folder, "vogelfreqs_w_lm_zlm_2024_doelsoorten"),
            devices, dpi = 300, width = 8, height = 7)
...
save_figure(p, here(media_folder, "cum_soorten_zoogdieren_w_lm_zlm_2024"),
            devices, dpi = 300, width = 8, height = 8)
```
...
Overzicht van relatieve frequenties in een bepaalde regio (gewogen gemiddelde).
Elk punt staat voor een relatieve frequentie in een bepaalde telperiode.
We nemen een gewogen gemiddelde per landbouwstreek waarbij de gewichten afhankelijk zijn van regio en stratum.
Over de telperiodes heen mogen we wel een ongewogen gemiddelde nemen want dit zijn replicaten.
...
Verschil in gemiddelde relatieve frequentie tussen de Westelijke zandleemstreek en de Westelijke leemstreek:
