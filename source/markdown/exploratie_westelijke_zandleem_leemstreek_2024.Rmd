...
Een overzicht van het aantal soorten per telpunt in de Westelijke leemstreek en de Zandstreek.
...
Voor verdere analyses groeperen we de Westelijke en Oostelijke Westelijke leemstreek tot de Polders.
...
Elk punt staat voor een relatieve frequentie in een bepaalde telperiode.
We nemen een gewogen gemiddelde per landbouwstreek waarbij de gewichten afhankelijk zijn van regio (in het geval van de Polders, westelijk vs. oostelijk) en stratum.
Over de telperiodes heen mogen we wel een ongewogen gemiddelde nemen want dit zijn replicaten.
...
relfreq_zoogdieren_regio <- relfreqs_zoogdieren %>%
  ...
  select(naam, Polders, Zandstreek, gewogen_gemiddelde = order_var) %>%
  arrange(desc(gewogen_gemiddelde)) %>%
  rownames_to_column(var = "rang") %>%
  mutate(rang = as.numeric(rang))
...
Top 20 van soorten die in termen van verschil in gemiddelde relatieve frequentie van voorkomen meer voorkomen in de Zandstreek dan in de Polders:
...
relfreqs_zoogdieren_diff <- relfreqs_zoogdieren %>%
  ...
  mutate(verschilfreq = `Zandstreek` - `Polders`) %>%
  ungroup() %>%
  arrange(desc(verschilfreq)) %>%
  rownames_to_column(var = "rang") %>%
  mutate(rang = as.numeric(rang)) %>%
  select(rang, naam, Zandstreek, Polders, verschilfreq)
