...
landbouwstreken <- landbouwstreken %>%
  mutate(
    Naam = ifelse(startsWith(NAAM, "Weidestreek"),
      "Weidestreek", NAAM
    ),
    section = NA
  ) %>%
...
> Er zijn in beide gebieden geen snelwegen aanwezig dus dit wordt niet in de functie gespecificeerd (anders krijgen we een error). De code verschilt dus licht van de code in de targets pipeline, maar zal hetzelfde resultaat geven.

> Er is in de Weidestreek geen vliegveld aanwezig dus dit wordt niet in de functie gespecificeerd (anders krijgen we een error). De code verschilt dus licht van de code in de targets pipeline, maar zal hetzelfde resultaat geven.
...
exclusie_osm_duinen <- exclusie_landgebruik_osm(
  ...
  landuse = c(
    "residential", "military", "industrial", "cemetery",
...
mapview::mapshot(m, file = here(
  "media", "steekproeftrekking",
...
```{r}
plot1 <- png::readPNG(here(
  "media", "steekproeftrekking",
  "afbakening_duinen_west.png"
))
plot2 <- png::readPNG(here(
  "media", "steekproeftrekking",
  "afbakening_duinen_oost.png"
))
...
telcirkels_selectie_landgbrk <- lapply(
  telcirkels_landgebruik, function(streek) {
...
selectie_landgebruik <- lapply(names(punten), function(i) {
  selectie_landgebruik_vito(
    punten_sf = punten[[i]],
    selectie_df = telcirkels_selectie_landgbrk[[i]]
  )
})
...
exclusie_punten <- lapply(names(punten), function(i) {
  punten[[i]] %>%
    left_join(
      telcirkels_selectie_landgbrk[[i]] %>%
        group_by(pointid) %>%
...
```{r}
telcirkels_selectie_landgbrk$Duinen %>%
  ggtern::ggtern(aes(
    x = Akker + Grasland,
    y = Bos + Struikgewas,
    z = Overige
...
mapview(
  ...
  mapview(paden$Duinen, legend = FALSE) +
  mapview(exclusie_punten$Duinen,
    col.regions = "black",
    legend = FALSE
  )
```
...
```{r}
telcirkels_selectie_landgbrk$Weidestreek %>%
  ggtern::ggtern(aes(
    x = Akker + Grasland,
    y = Bos + Struikgewas,
    z = Overige
...
mapview(
  ...
  mapview(paden$Weidestreek, legend = FALSE) +
  mapview(exclusie_punten$Weidestreek,
    col.regions = "black",
    legend = FALSE
  )
```
...
In de Duinen zien we dat het steekproefkader vooral in twee gebieden gelokaliseerd is.
...
m <- mapview(
  ...
  mapview(detail_west_sf,
    color = "purple", alpha.regions = 0, legend = FALSE,
    lwd = 2
  )
...
steekproefkader_per_stratum <- lapply(
  names(steekproefkader_finaal),
  function(i) {
...
allocatie_df$Duinen %>%
  select(
    Naam, openheid_klasse, is_sbp, popsize, allocatie_gebied,
...
```{r}
eerste_set_weidestreek <- nrow(filter(
...
for (i in seq_along(sizes)) {
  ...
  out_list[[i]] <- tibble(
    ...
    chisq = calc_chisq(
      observed = summary_df$n_steekproef,
...
for (j in seq_along(thinning)) {
  ...
  for (i in seq_along(sizes)) {
    ...
    print(paste0(
      "Steekproefgrootte: ", extended_size,
      ". Thinning: ", thin, "."
    ))
...
We bekijken de steekproefgroottes met Chi-kwadraattoetsingsgrootheid kleiner dan 1 en finale steekproefgrootte minstens 50:

```{r}
chi_squared_df_thinning %>%
  filter(
    chisq < 1,
    finale_grootte >= 50
  ) %>%
  arrange(desc(finale_grootte)) %>%
  kable()
```

We besluiten een steekproeftrekking van 91 punten te nemen om 58 punten over te houden bij een thinning van  400 m.
...
Er is inderdaad een goede overeenkomst tussen de proporties in de steekproef (`prop_steekproef`) en de originele allocatie (`allocatie`).
...
```{r}
balansvergelijking_wd2 <- steekproef_extended_thinned %>%
  select_at(names(steekproefkader_finaal$Weidestreek)) %>%
  mutate(fill_var = "steekproef") %>%
  bind_rows(steekproefkader_finaal$Weidestreek %>%
              mutate(fill_var = "steekproefkader")) %>%
...
count_df_weidestreek2 <- balansvergelijking_wd2 %>%
  group_by(fill_var, openheid_klasse, sbp_akkervogels) %>%
  summarise(
    max_openheid = max(openheid_waarde),
    max_x = max(X),
...
We zien vooral dat de steekproef relatief goed overeenkomt met het steekproefkader op vlak van openheid.

```{r}
balansvergelijking_wd2 %>%
  ggplot(aes(x = sbp_akkervogels, y = openheid_waarde)) +
  geom_boxplot(aes(fill = fill_var)) +
  geom_label(
    data = count_df_weidestreek2,
    aes(y = max_openheid + 0.01, label = paste("n =", n)),
    size = 3, position = position_dodge2(width = 0.75)
  ) +
  facet_grid(~openheid_klasse) +
...
Als we naar longitude en latitude kijken, lijkt dit wel min of meer ok. Opnieuw is er voor OL buiten SBP niet zo'n goede match, maar dit is ook maar 1 punt dus het is onmogelijk een goede vergelijking te maken.
