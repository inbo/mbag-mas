...
setdiff(tolower(indicator_species_nl),
        tolower(unique(abv_indicator[["vernacularName"]])))
```
...
| aantal tellingen per locatie | afzonderlijke telpunten | 6 telpunten per km-hok |
| duur telling | 10' per telpunt | 5' per telpunt |
| afstand | 300 m | ongelimiteerd |

We zien dat het moeilijk is beide meetnetten te vergelijken omdat ze verschillen in detectiekans omdat de inspanningen niet hetzelfde zijn.
...
We vermenigvuldigen de aantallen van ABV daarom met $4/3$.

```{r}
# Count number of points ABV
abv_telpunten2 <- abv_indicator %>%
  summarise(aantal_kmhokken_abv = n_distinct(UTM) * (4 / 3),
            tot_hokken_abv = tot_points_abv,
            prop_kmhokken_abv = aantal_kmhokken_abv / tot_hokken_abv,
            .by = naam)

# Join numbers
full_join(mas_telpunten, abv_telpunten2, by = join_by(naam)) %>%
  mutate(tot_hokken_abv = tot_points_abv) %>%
  replace_na(list(aantal_kmhokken_abv = 0, prop_kmhokken_abv = 0)) %>%
  select(
    naam, starts_with("aantal"), starts_with("prop"), starts_with("tot_")
...
# Join numbers
full_join(mas_individuals, abv_individuals2, by = join_by(naam)) %>%
  mutate(tot_hokken_abv = tot_points_abv) %>%
  replace_na(list(aantal_individuen_abv = 0, rel_aantal_abv = 0)) %>%
  select(
    naam, starts_with("aantal"), starts_with("rel"), starts_with("tot_")
