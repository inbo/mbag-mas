...
# Which
setdiff(indicator_species_nl, unique(abv_indicator_full[["vernacularName"]]))
```
...
abv_data_post2016 %>%
  ...
  select(
    UTM, year, Stratum, individualCount, eventDate, level3Name, level2Name
  ) %>%
  arrange(UTM, eventDate, desc(year), level3Name, level2Name) %>%
  kable()
```

Ze werd wel gezien in vorige jaren in 'landbouw-hokken'.
