...
mas_telpunten <- mas_2024_indicator %>%
  ...
  summarise(aantal_telpunten_mas = n_distinct(plotnaam),
            prop_telpunten_mas = aantal_telpunten_mas / tot_points_mas,
            .by = naam)
...
abv_telpunten <- abv_indicator %>%
  summarise(aantal_kmhokken_abv = n_distinct(UTM),
            prop_kmhokken_abv = aantal_kmhokken_abv / tot_points_abv,
            .by = naam)

# Join numbers
full_join(mas_telpunten, abv_telpunten, by = join_by(naam)) %>%
  select(naam, starts_with("aantal"), starts_with("prop")) %>%
