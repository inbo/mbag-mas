---
title: "MAS steekproefontwerp kempen"
...
targets_store <- here("source", "targets", "mas_steekproef_za_kp_po",
                      "_targets")
```
...
Om de perimeter te bepalen voer je onderstaande code uit in `mas_steekproef_za_kp_po.Rproj`, na laden van globals in `targets_steekproef_za_kp_po.Rmd`.

De code maakt een intersectie tussen landbouwstreek Kempen (heel België) en contour Vlaanderen en voegt deze toe aan file `za_kp_po_perimeters.gpkg`.

```r
# library(sf)
#
# # Lees in polygonen landbouwstreken
# landbouwstreken <- read_sf(file.path(mbag_dir,
#                                      "data",
#                                      "landbouwstreken",
#                                      "Lbstrbel.shp")) |>
...
#   st_transform(crs = 31370)
#
# kemp_vlaanderen <- st_intersection(landbouwstreken, vlaanderen) |>
#   filter(NAAM == "Kempen") |>
#   mutate(section = NA) |>
#   select(Naam = NAAM, section, geom = geometry)
#
# st_write(obj = kemp_vlaanderen,
#          dsn = path_to_perimeters("za_kp_po_perimeters.gpkg"),
#          append = TRUE)
```
...
mapview(st_crop(perimeters_data, detail),
  color = "red", alpha.regions = 0,
  legend = FALSE
) +
  mapview(detail_sf, color = "purple", alpha.regions = 0, legend = FALSE) +
  mapview(selectie_openheid_klasses_crop,
    col.regions = "red",
    legend = FALSE
  ) +
  mapview(st_crop(exclusie_osm_landgebruiken, detail),
    col.regions = "green",
    legend = FALSE
  )
```
...
```{r}
telcirkels_selectie_lgb <- tar_read(telcirkels_selectie_landgebruik,
  store = targets_store
) %>%
  filter(startsWith(pointid, abbreviate("Kempen", 2)))
```
...
mapview(st_crop(perimeters_data, detail),
  ...
  mapview(st_crop(paden, detail), legend = FALSE) +
  mapview(st_crop(exclusie_punten, detail),
    col.regions = "black",
    legend = FALSE
  )
```
...
Eventueel verder opgesplitst in winter, zomer en jaarrond maatregelen.
...
-   SBP's andere soorten (download 2023-02-14): <https://www.vlaanderen.be/datavindplaats/catalogus/soortenbeschermingsprogrammas>
...
Het oppervlakte aandeel aan SBP's binnen de Kempen:
...
Het oppervlakte aandeel aan SBP's binnen OL en HOL in de Kempen:
...
mapview(st_crop(perimeters_data, detail),
  ...
  mapview(st_crop(paden, detail), legend = FALSE) +
  mapview(st_crop(steekproefkader_finaal_strata, detail),
    zcol = "stratum",
    layer = "stratum"
  )
```

# Trekking steekproef Kempen
...
-   Hier moeten we afweging maken tussen de precisie per stratum en precisie voor de Kempen in zijn geheel.
...
In het algemeen kunnen we voor een naar kosten optimale allocatie gebruik maken van een formule die stelt dat de allocatie recht evenredig is met de oppervlakte van een stratum en de variabiliteit in een stratum en omgekeerd evenredig is met de vierkantswortel van de kosten van het veldwerk in het stratum.
...
Er zijn 4 strata (OL en HOL: binnen en buiten SBP) binnen de Kempen.
...
De eerste bv. 300 (laagste volgnummers) zullen dan de set zijn voor de Kempen, die kan verdeeld worden over de medewerkers.
...
first_samples <- function(steekproef_df, min_n, multiplicatie) {
  ...
  steekproef_df %>%
    group_by(.data$openheid_sbp, .data$thin_dist) %>%
    mutate(id = row_number()) %>% # sample_order cannot be used due to thinning
    ungroup() %>%
    filter(id <= factor_n) %>%
    mutate(batch = ifelse(id <= min_n, .data$batch, "reserve set")) %>%
    select(-id) %>%
...
balansvergelijking %>%
  ggplot() +
  geom_boxplot(aes(
    x = sbp_akkervogels, y = openheid_waarde,
    fill = fillvar
  )) +
...
Een confounding probleem is dat de SBP's gemiddeld nogal noordelijk gelegen zijn dan.
...
mapview_n <- function(n) {
  mapview::mapview(perimeters_data, color = "red", alpha.regions = 0,
                   legend = FALSE) +
    mapview::mapview(
      first_samples(steekproef, n, multiplicatie = 1) %>%
        st_buffer(dist = 300),
...
3.  Gittargets
    ...
    -   Is het de moeite? Manueel werk --\> waarom niet ineens hard coden? Komt op zelfde neer? <!-- spell-check: ignore -->
...
We vervangen alle telpunten uit de steekproef van de Kempen door reeds bestaande punten indien ze voor minstens 50 % overlappen en ze tot hetzelfde stratum behoren.
...
intersect <- st_intersection(oude_cirkels_buffer, cirkels_kempen) %>%
  mutate(
...
Controle bestaande punten in steekproef:
```{r}
steekproef %>%
  filter(!str_detect(pointid, "Km")) %>%
  kable()
```
...
mapview(steekproef %>% st_buffer(300),
  alpha.regions = 0,
  layer = "steekproef"
) +
  mapview(steekproef_out %>% st_buffer(300),
    zcol = "batch",
...
steekproef_out %>%
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_kempen_telpunten.geojson"
    ),
    delete_dsn = TRUE
  )
...
steekproef_out %>%
  ...
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_kempen_telpunten.kml"
    ),
    delete_dsn = TRUE
  )
...
steekproef_out %>%
  ...
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_kempen_telcirkels.geojson"
    ),
    delete_dsn = TRUE
  )
...
steekproef_out %>%
  ...
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_kempen_telcirkels.kml"
    ),
    delete_dsn = TRUE
  )
```
...
kempen_avimap_df <- read_delim(
  ...
  filter(beslissing %in% c("behouden", "vervangen door bestaand")) %>%
...
kempen_avimap_sf %>%
  st_write(
    dsn = file.path(
      "data",
      "processed",
      "steekproef_kempen_avimap.geojson"
    ),
    delete_dsn = TRUE
  )
```
...
```{r, eval=FALSE}
git2rdata::write_vc(kempen_avimap_df,
  file = file.path(
    "data",
    "processed",
    "steekproef_kempen_avimap"
  ),
  sorting = "pointid"
)
```
