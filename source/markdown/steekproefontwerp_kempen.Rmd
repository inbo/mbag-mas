...
targets_store <- here("source", "targets", "mas_steekproef_zl_lm_za_kp_po", "_targets")
```
...
De code maakt een intersectie tussen landbouwstreek Kempen (heel België) en contour Vlaanderen en voegt deze toe  aan file "zandleemstreek_perimeters.gpkg".
...
#          dsn = path_to_perimeters("zl_lm_za_kp_po_perimeters.gpkg"),
#          append = TRUE)
```
...
We berekenen de cumulatieve zichtbaarheidsindex waarbij de waarnemer een bewegingsvrijheid heeft van 25 m rondom het telpunt.
...
Ward, is dit niet een zeer lage cut-off?
...
Er zijn 4 strata (OL, HOL en binnen, buiten SBP) binnen de Kempen.
...
intersect <- st_intersection(oude_cirkels_sbp, cirkels_kempen) %>%
