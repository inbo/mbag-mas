...
In tweede instantie sluiten we een aantal landgebruiken uit op basis van OpenStreetMap data [@openstreetmapcontributors2024] (Tabel @tab-exclusie-landgebruik), Fig.
...
We gebruiken daarom een extractie uit OpenStreetMap van alle wegentypes die in aanmerkingen komen (positieve selectie, dus snelwegen en dergelijke worden hier indirect uitgesloten) (Tabel @tab-inclusie-wegen, Fig. @fig-steekproef-afbakening C):
...
Voor het SBP gebied combineren we meerdere SBP lagen (zie Bijlage \textcolour{red}{sbp-per-soort}): (1) perimeter SBP akkervogels [@jansen2022adviesakkervogels], (2) SBP grauwe kiekendief (oude perimeter van in gebruik van 2018 t/m 2022 [@vandegehuchte2015grauwekiekendief] en nieuwe (door INBO voorgestelde) perimeter [@spanoghe2022grauwekiekendief]), en (3) SBP bruine kiekendief, zomertortel en hamster [@overigeSBPs].
...
In totaal worden `r nrow(steekproef_final)` telpunten geteld in kader van MAS Vlaanderen (Tabel @tab-aantal-telpunten).
Figuur @fig-mas-steekproef-tot toont de verspreiding van deze punten.

```{r aantal-telpunten}
#| label: tbl-aantal-telpuntent
