...
Dit meetnet wordt opgebouwd op basis van de verschillende landbouwstreken in Vlaanderen (Fig. @fig-landbouwstreken).
...
In dit hoofdstuk vatten we eerst samen hoe het meetnet is opgebouwd en de steekproeftrekking werd gedaan (Sectie @sec-meetnet-ontwerp).
Daarna overlopen we welke stappen voor MAS Vlaanderen reeds werden gedaan (Zandleemstreek en Leemstreek) \textcolor{red}{[REF]} en welke uitbreiding er in 2024 gepland is (Polders, Zandstreek, Kempen) (Sectie @sec-mas-vlaanderen).
Ten slotte bespreken we de overige, kleine streken (Duinen, Weidestreek) apart in Sectie @sec-overige-streken.
...
We bekomen dus polygonen voor elke landbouwstreek in Vlaanderen[^05_uitrol_meetnet-1].
...
Binnen deze polygonen selecteren we open en halfopen landschap (Fig. @fig-steekproef-afbakening A).
...
In tweede instantie sluiten we een aantal landgebruiken uit op basis van OpenStreetMap data [@openstreetmapcontributors2024] (Tabel @tbl-exclusie-landgebruik), Fig.
@fig-steekproef-afbakening B).
...
We gebruiken daarom een extractie uit OpenStreetMap van alle wegentypes die in aanmerkingen komen (positieve selectie, dus snelwegen en dergelijke worden hier indirect uitgesloten) (Tabel @tbl-inclusie-wegen, Fig. @fig-steekproef-afbakening C):
...
Vervolgens hebben we dit netwerk van wegen omgevormd tot een puntennetwerk (Fig. @fig-steekproef-afbakening D), waarbij elk punt een potentieel telpunt kan zijn:
...
Maar ook binnen een landbouwstreek willen we verder stratificeren in termen van binnen en buiten **soortbeschermingsplan zones (SBP)** voor akkervogels en in termen van **openheid van het landschap** (Fig. @fig-stratificatievariabelen).
...
<!-- Voor het SBP gebied combineren we meerdere SBP lagen (zie Bijlage \textcolor{red}{sbp-per-soort}): (1) perimeter SBP akkervogels [@jansen2022adviesakkervogels], (2) SBP grauwe kiekendief (oude perimeter van in gebruik van 2018 t/m 2022 [@vandegehuchte2015grauwekiekendief] en nieuwe (door INBO voorgestelde) perimeter [@spanoghe2022grauwekiekendief]), en (3) SBP bruine kiekendief, zomertortel en hamster [@overigeSBPs]. --> Deze stratificatie is niet zwart-wit.
...
De berekening voor openheid van het landschap is dezelfde als uitgelegd in de eerste stap van Sectie @sec-mas-steekproefkader.
...
Een ander voordeel van deze methode is dat elk getrokken telpunt een volgnummer krijgt, wat van belang was bij het nagaan van overlap met bestaande telpunten en bij de manuele controle (Sectie @sec-nabehandeling).
...
De precieze verdeling en aantal punten per landbouwstreek word besproken in Sectie @sec-mas-vlaanderen.
...
Deze zijn apart geanalyseerd omdat het niet mogelijk is een voldoende grote steekproef te verkrijgen op eenzelfde wijze als beschreven in Sectie @sec-meetnet-ontwerp.
...
In het resterende gebied ligt 51,67 km paden en wegen (Fig. @fig-afbakening-duinen).
...
In het overgebleven steekproefkader merkten we een sterke spatiale clustering van punten zowel in zijn geheel (enkel punten in regio's Nieuwpoort en Knokke) als binnen eenzelfde stratum (Fig. @fig-steekproefkader-duinen).
...
In het resterende gebied ligt 80,56 km paden en wegen (Fig. @fig-afbakening-weidestreek).
...
Langs deze paden werden punten gelegd die ten slotte werden gefilterd op basis van landgebruik en zichtbaarheid zoals beschreven in Sectie @sec-meetnet-ontwerp.
In het overgebleven steekproefkader merkten we een sterke spatiale clustering van punten binnen eenzelfde stratum, maar met een relatief goede spreiding van de punten in het volledige gebied (Fig. @fig-steekproefkader-weidestreek).
...
We besluiten in de Weidestreek wel een steekproef te nemen zoals beschreven in Sectie @sec-meetnet-ontwerp met het verschil dat we hier een allocatie gebruikten evenredig aan de oppervlakte van elk stratum.
...
Voor alle andere landbouwregio's werd een afstand van 600 m gehanteerd zodat er geen overlap is tussen telpunten (zie Sectie @sec-nabehandeling).
...
In totaal worden `r nrow(steekproef_final)` telpunten geteld in kader van MAS Vlaanderen (Tabel @tbl-aantal-telpunten).
Figuur @fig-mas-steekproef-tot toont de verspreiding van deze punten.
...
[^05_uitrol_meetnet-1]: Met uitzondering van de Duinen en de Weidestreek.
    Deze worden apart besproken in Sectie @sec-overige-streken.
