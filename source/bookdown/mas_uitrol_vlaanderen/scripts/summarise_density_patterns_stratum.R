...
#'
#' Generates a narrative description of density estimates across regions and years,
#' highlighting general patterns (OL vs HOL, binnen vs buiten SBP),
...
#'     \item{sbp}{Factor with values \code{"binnen"} or \code{"buiten"} (inside/outside SBP).}
...
#'   \describe{
#'     \item{effect}{Categorical: \code{"boven"}, \code{"onder"}, or \code{"geen sign.\nverschil"}.}
...
summarise_density_patterns_stratum <- function(data, threshold = 1.5) {
  # Check that required columns are present
  stopifnot(all(c("regio", "openheid", "sbp", "estimate", "lcl", "species", "year") %in% names(data)))
  ...
  # Function to summarise OL vs HOL and binnen vs buiten per region for a given year
  summarize_year <- function(subdata) {
    ol_vs_hol <- bin_vs_bui <- setNames(vector("list", length(regions)), regions)
    for (reg in regions) {
      ...
      sum_ol <- sum(df$estimate[df$openheid == "OL" & df$present], na.rm = TRUE)
      sum_hol <- sum(df$estimate[df$openheid == "HOL" & df$present], na.rm = TRUE)
      ol_vs_hol[[reg]] <- if (sum_ol > threshold * sum_hol) "OL" else if (sum_hol > threshold * sum_ol) "HOL" else "similar"

      # Compare binnen vs buiten SBP using sum of densities
      sum_bin <- sum(df$estimate[df$sbp == "binnen" & df$present], na.rm = TRUE)
      sum_bui <- sum(df$estimate[df$sbp == "buiten" & df$present], na.rm = TRUE)
      bin_vs_bui[[reg]] <- if (sum_bin > threshold * sum_bui) "binnen" else if (sum_bui > threshold * sum_bin) "buiten" else "similar"
    }
    list(ol_vs_hol = ol_vs_hol, bin_vs_bui = bin_vs_bui)
  }

  # Compute OL/HOL and SBP patterns per year
  patterns_by_year <- lapply(years, function(y) summarize_year(data[data$year == y, ]))
  names(patterns_by_year) <- years

  # Check if OL/HOL and SBP patterns are consistent across years
  ol_consistent <- all(sapply(patterns_by_year[-1], function(p) identical(p$ol_vs_hol, patterns_by_year[[1]]$ol_vs_hol)))
  bin_consistent <- all(sapply(patterns_by_year[-1], function(p) identical(p$bin_vs_bui, patterns_by_year[[1]]$bin_vs_bui)))

  text_parts <- list()

  # Absence statement for regions absent in all years
  if (length(absent_regions) > 0) {
    text_parts <- c(text_parts, sprintf("De soort is praktisch afwezig in %s.", paste(absent_regions, collapse = ", ")))
  }
  ...
  if (ol_consistent && bin_consistent) {
    ...
    regional_text <- mapply(describe_region, names(combined$ol_vs_hol), combined$ol_vs_hol, combined$bin_vs_bui, SIMPLIFY = TRUE)
  ...
  for (y in years) {
    ...
    highest_text <- sprintf(
      "In %s werd de hoogste dichtheid gevonden in %s in %s, %s SBP (%.2f [%.2f–%.2f] broedparen/100 ha).",
    ...
    if (min_row$estimate > 0) {
      lowest_text <- sprintf(
        "De laagste dichtheid in %s werd gevonden in %s in %s, %s SBP (%.2f [%.2f–%.2f] broedparen/100 ha).",
  ...
  if ("effect" %in% names(data) && "threshold" %in% names(data)) {
    ...
    if (all(data$effect == "onder")) {
      text_parts <- c(
        text_parts,
        sprintf("Als we vergelijken met de streefwaarde van @agentschapvoornatuurenbos2021, nl. %s broedparen per 100 ha, zien we dat alle densiteiten significant lager zijn dan de streefwaarde.", gsub("\\.", ",", t))
      )
    } else {
      # Start threshold section
      threshold_text <- sprintf("We vergelijken met de streefwaarde van @agentschapvoornatuurenbos2021, nl. %s broedparen per 100 ha.", gsub("\\.", ",", t))
      ...
      if (nrow(boven_rows) > 0) {
        boven_text <- apply(boven_rows, 1, function(row) {
          sprintf("De densiteit ligt significant hoger dan de streefwaarde in %s in %s in %s, %s SBP",
                  row[["year"]], row[["regio"]], row[["openheid"]], row[["sbp"]])
        })
        text_parts <- c(text_parts, paste(boven_text, collapse = "; "))
      } else {
        text_parts <- c(text_parts, "De densiteiten liggen in geen enkel geval significant hoger dan de streefwaarde.")
      }
      ...
      if (nrow(geen_rows) > 0) {
        geen_text <- apply(geen_rows, 1, function(row) {
          sprintf("%s in %s in %s, %s SBP", row[["year"]], row[["regio"]], row[["openheid"]], row[["sbp"]])
        })
        text_parts <- c(text_parts, "We zien geen significant verschil met de streefwaarde in", paste0(paste(geen_text, collapse = "; "), "."))
      }

      # Remaining cases are under
      text_parts <- c(text_parts, "In alle andere gevallen is de densiteit significant lager dan de streefwaarde.")
    }
  }
