...
output_files2 <- paste0("_", gsub("\\s", ".", tolower(species)), "2", ".qmd")

for (i in seq_along(species)) {
  spec <- species[i]
  lab <- gsub("\\s", ".", tolower(spec))
  out_file <- output_files[i]
  out_file2 <- output_files2[i]
  ...
  knit_expand(
    "_species_detectioncurves.qmd", species = spec, label = lab
  ) |>
    writeLines(file.path(spec_dir, out_file2))
}
...
# -------------------------------
# Read the QMD
lines <- readLines("09_densiteitsschattingen.qmd")
...
# Write back to QMD
writeLines(lines_new, "09_densiteitsschattingen.qmd")

## Appendix
# Read the QMD
lines <- readLines("detection_curves.qmd")
...
# Write back to QMD
writeLines(lines_new, "detection_curves.qmd")
