...
We kijken naar het aantal individuen per regio per broedcode.

```{r}
filtered_distance_df %>%
  filter(jaar == params$year) %>%
  ggplot(aes(x = periode_in_jaar)) +
  geom_bar() +
...
We kijken naar het aantal individuen per stratum.
...
filtered_distance_df %>%
  ...
  geom_bar() +
  facet_wrap(~regio, scales = "free") +
  labs(x = "Telperiode", y = "Aantal individuen", fill = "Stratum",
       title = params$year)
```
...
filtered_distance_df %>%
  ...
  geom_histogram() +
  facet_wrap(~regio, scales = "free") +
  labs(x = "Afstand (m)", y = "Aantal individuen", fill = "Stratum",
       title = params$year)
```
...
maxima_df %>%
  ...
  geom_boxplot() +
  labs(x = "", y = "Aantal individuen per telpunt\n(maximum over telperiodes)",
       title = params$year) +
