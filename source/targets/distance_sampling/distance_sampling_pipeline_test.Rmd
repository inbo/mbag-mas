...
```{r}
params$year <- 2024
params$species <- "Roodborsttapuit"
```
...
sim_out
simulationOutput2 <- recalculateResiduals(sim_out, group = count_data_2023$plotnaam)
...
gam_res <- gam(rsd ~ s(x_plot, y_plot, bs = "tp") - 1,
          data = count_data_2023, select = TRUE)
gam_res
```
...
library(glmmTMB)
fit_glmmTMB <- glmmTMB(
  count ~ stratum + periode_in_jaar + s(x_plot, y_plot, bs = "tp") + (1|plotnaam),
...
```{r}
m_gp <- gam(count ~ stratum + periode_in_jaar + s(x_plot, y_plot, bs = "gp", m = 3) +
            s(plotnaam, bs = "re"),
          data = count_data_2023, family = poisson())
```
...
```{r}
preds <- predict(fit_glmmTMB, newdata = prediction_hexgrid %>% mutate(plotnaam = NA), type = "response")
```
  ...
  scale_fill_gradientn(
      ...
      values = scales::rescale(
          c(min(pred_dens$pred_dens, na.rm = TRUE),
            ...
            mean(pred_dens$pred_dens, na.rm = TRUE) + 0.2*diff(range(pred_dens$pred_dens, na.rm = TRUE)),
