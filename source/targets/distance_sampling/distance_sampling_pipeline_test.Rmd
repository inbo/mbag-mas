...
> herhaal broedseizoen voor deze soort

```{r}
filtered_distance_df <- tar_read_ds("filtered_breeding_date", "Veldleeuwerik")
```
...
filtered_distance_df %>%
  ...
  labs(x = "Telperiode", y = "Aantal broedparen", fill = "Broedcode", title = y)
```
...
filtered_distance_df %>%
  ...
  geom_bar() +
  facet_wrap(~regio, scales = "free") +
  labs(x = "Telperiode", y = "Aantal broedparen", fill = "Stratum", title = y)
```
...
filtered_distance_df %>%
  ...
  geom_histogram() +
  facet_wrap(~regio, scales = "free") +
  labs(x = "Afstand (m)", y = "Aantal broedparen", fill = "Stratum", title = y)
```

We selecteren het maximum aantal individuen per telpunt.

```{r}
obs_table <- tar_read_ds("obs_table", "Veldleeuwerik") %>%
           filter(year == y)
...
zeroes_df <- sample_table %>%
  ...
  select("plotnaam" = "Sample.Label", "jaar" = "year", "stratum" = "Region.Label") %>%
...
area_table_stratum <- area_table_stratum %>%
  separate(Region.Label, into = c("regio", "openheid_klasse", "sbp"),
           sep = " - ") %>%
  rename("area" = "Area")
...
area_table_region <- area_table_region %>%
  bind_rows(
    tibble(
      Region.Label = "Total",
      Area = sum(area_table_region$Area)
    )
  ) %>%
...
```{r}
nc_df %>%
  left_join(a_df, by = join_by(jaar, regio, openheid_klasse, sbp)) %>%
  left_join(area_table_stratum, by = join_by(regio, openheid_klasse, sbp)) %>%
  rowwise() %>%
  mutate(
    abundantie = (area / sample_area) * nc,
    densiteit = nc / sample_area
  ) %>%
  arrange(jaar, regio, openheid_klasse, sbp) %>%
  select(jaar, regio, openheid = openheid_klasse, sbp, abundantie, densiteit) %>%
  kable(digits = 2)
```
